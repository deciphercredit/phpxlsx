 ...
 */
require_once dirname(__FILE__) . '/../Logger/PhpxlsxLogger.php';
...
class SignXlsx implements Sign
{
    ...
    public function setXlsx($file)
    {
        ...
        if ($this->xlsx->open($file) === TRUE) {
            for ($i = 0; $i < $this->xlsx->numFiles; $i++) {
                $fileName = $this->xlsx->getNameIndex($i);
                $this->_xlsxStructure[$this->xlsx->getNameIndex($i)] = $this->xlsx->getFromName($fileName);
            }
        } else {
    ...
    {
        $this->_privatekey = openssl_pkey_get_private(file_get_contents($file), $password);
        if (!$this->_privatekey) {
            PhpxlsxLogger::logger('Unable to find a private key.', 'fatal');
        }
    }
    ...
    public function setX509Certificate($file)
    {
        if (is_file($file)) {
            ...
            if (!$this->_x509Certificate) {
                PhpxlsxLogger::logger('Unable to find a certificate.', 'fatal');
            }
            ...
            PhpxlsxLogger::logger('The file does not exist', 'fatal');
        }
    }
    ...
    public function sign()
    {
        ...
        $relsRelsContent = $this->_xlsxStructure['_rels/.rels'];
        ...
        if (!isset($this->_xlsxStructure['_xmlsignatures/origin.sigs'])) {
        ...
        $this->_xlsxStructure['_rels/.rels'] = $seRelsRelsContent->asXML();
        ...
        $contentTypesContent = $this->_xlsxStructure['[Content_Types].xml'];
        if (PHP_VERSION_ID < 80000) {
            $optionEntityLoader = libxml_disable_entity_loader(true);
        }
        ...
        $this->_xlsxStructure['[Content_Types].xml'] = $seContentTypesContent->asXML();
        ...
        $wordRelsDocumentContent = $this->_xlsxStructure['xl/_rels/workbook.xml.rels'];
        if (PHP_VERSION_ID < 80000) {
            $optionEntityLoader = libxml_disable_entity_loader(true);
        }
        ...
        foreach ($this->_xlsxStructure as $xlsxContentKey => $xlsxContentValue) {
            if (preg_match('/^xl.*\.xml.rels$/', $xlsxContentKey) || preg_match('/^xl.*\.vml.rels$/', $xlsxContentKey)) {
                $wordRels = $xlsxContentValue;
                if (PHP_VERSION_ID < 80000) {
                    $optionEntityLoader = libxml_disable_entity_loader(true);
                }
                ...
                $elementsRelationRelsDocumentContent = $seWordRels->xpath('//r:Relationship');
                foreach ($elementsRelationRelsDocumentContent as $elementRelation) {
                    $domElementRelation = dom_import_simplexml($elementRelation);
                    // avoid externals relationships
                    if ($domElementRelation->getAttribute('TargetMode') == 'External' || $domElementRelation->getAttribute('Type') == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/customXml') {
                        continue;
                    }

                    if (!in_array('xl/' . $domElementRelation->getAttribute('Target'), $filesXMLToBeAdded)) {
                        $target = $domElementRelation->getAttribute('Target');
                        if ($domElementRelation->getAttribute('Type') == 'http://schemas.microsoft.com/office/2011/relationships/chartColorStyle' || $domElementRelation->getAttribute('Type') == 'http://schemas.microsoft.com/office/2011/relationships/chartStyle') {
                            if (strstr($xlsxContentKey, 'xl/charts/')) {
                                $target = 'charts/' . $target;
                            }
                        }
                        $target = str_replace('../', '', $target);
                        $filesXMLToBeAdded[] = 'xl/' . $target;
                    }
                }

                if (isset($this->_idsRels[$xlsxContentKey])) {
                    $digestValue = $this->calculateDigestValueTransform($seWordRels, $this->_idsRels[$xlsxContentKey]);
                    $this->addReference('/' . $xlsxContentKey . '?ContentType=application/vnd.openxmlformats-package.relationships+xml', $digestValue, 'http://www.w3.org/2000/09/xmldsig#sha1', true, $this->_idsRels[$xlsxContentKey]);
                }
            }
        }
        ...
        foreach ($filesXMLToBeAdded as $fileXMLToBeAdded) {
            ...
            $digestValue = $this->calculateDigestValue($this->_xlsxStructure[$fileXMLToBeAdded]);
            // add reference
        ...
        // close ZIP file
        foreach ($this->_xlsxStructure as $key => $value) {
            $this->xlsx->addFromString($key, $value);
        }
        $this->xlsx->close();
    }
    ...
    public function transformPfxToPem($source, $dest, $password = null)
    {
        ...
        if ($worked) {
            ...
            if ($worked) {
                ...
                PhpxlsxLogger::logger($log_error, 'fatal');
            }
        } else {
            $log_error = openssl_error_string();
            PhpxlsxLogger::logger($log_error, 'fatal');
    ...
    {
        if (!isset($this->_xlsxStructure['_xmlsignatures/_rels/origin.sigs.rels'])) {
            $this->_xlsxStructure['_xmlsignatures/_rels/origin.sigs.rels'] = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId'.$idTempNumericSig.'" Type="http://schemas.openxmlformats.org/package/2006/relationships/digital-signature/signature" Target="sig'.$idTempNumericSig.'.xml"/></Relationships>';
        } else {
            $originSigsRelsContent = $this->_xlsxStructure['_xmlsignatures/_rels/origin.sigs.rels'];
            ...
            $this->_xlsxStructure['_xmlsignatures/_rels/origin.sigs.rels'] = $originSigsRelsContent;
        }

        if (!isset($this->_xlsxStructure['_xmlsignatures/origin.sigs'])) {
            $this->_xlsxStructure['_xmlsignatures/origin.sigs'] = '';
        }

        $this->_xlsxStructure['_xmlsignatures/sig'.$idTempNumericSig.'.xml'] = $this->_xml->asXML();
    }
    ...
    private function calculateDigestValueTransform($string, $IdsArray)
    {
        ...
        foreach ($IdsArray as $key => $value) {
            ...
            $xlsxXPath = new \DOMXPath($doc);
            $xlsxXPath->registerNamespace('rels', 'http://schemas.openxmlformats.org/package/2006/relationships');

            $node = $xlsxXPath->query($query)->item(0);
    ...
    private function returnIdsRels()
    {
        ...
        foreach ($this->_xlsxStructure as $xlsxContentKey => $xlsxContentValue) {
            if (preg_match('/^.*\.(rels)$/', $xlsxContentKey)) {
                if (PHP_VERSION_ID < 80000) {
                    $optionEntityLoader = libxml_disable_entity_loader(true);
                }
                $xmlContent = simplexml_load_string($xlsxContentValue);
                if (PHP_VERSION_ID < 80000) {
                    libxml_disable_entity_loader($optionEntityLoader);
                }
                ...
                if ($elementsRelationship) {
                    ...
                    $this->_idsRels[$xlsxContentKey] = $idsRelsTemp;
                }
            }
        }
    }
    ...
    private function setSignatureValue()
    {
        ...
        openssl_sign($domIdSignedInfo->C14N(), $signDigestValue, $this->_privatekey);
